<?php
    ...
    class aplikimetRepository{
        ...
        public function insertAplikimet($aplikimetpunes){
            ...
            $sql = "INSERT INTO aplikimetpunes(emri,mbiemri,email, nenshtetsia,adresa) VALUES (?,?,?,?,?)";

            $statement = $conn->prepare($sql);
            $statement->execute([$emri, $mbiemri, $email, $nenshtetsia, $adresa]);
        ...
        public function editAplikimetPunes($id, $emri, $mbiemri, $email, $nenshtetsia, $adresa){
            $conn = $this->connection;
            $sql = "UPDATE aplikimetpunes SET emri=?,mbiemri=?, email=?, nenshtetsia=?,adresa=? WHERE Id=?";

            $statement = $conn->prepare($sql);
            $statement->execute([$emri,$mbiemri, $email, $nenshtetsia, $adresa,$id]);
